...
file_myopathy = "emg_myopathy.txt";
GAIN = 1;
...
[timeMyopathy, amplitudeMyopathy] = process_EMG_data(file_myopathy, GAIN);
...
filteredEMG_myopathy = apply_bandpass_filter(amplitudeMyopathy, [20 498], SAMPLING_RATE);
...
%Process signals for myopathy EMG
[amplitudeMyopathy_offset, filteredEMG_myopathy_offset, rectified_myopathy] = offset_signal(amplitudeMyopathy, filteredEMG_myopathy);
...
start_indexM = find(timeMyopathy >= start_time, 1);
end_indexM = find(timeMyopathy <= end_time, 1, 'last');
...
rmsAmplitudeFullWindowMyopathy = rms(rectified_myopathy(start_indexM:end_indexM));
disp(['RMS Amplitude for the entire signal (Myopathy Signal): ', num2str(rmsAmplitudeFullWindowMyopathy)]);
...
mavValueFullWindowMyopathy = mean(abs(rectified_myopathy(start_indexM:end_indexM)));
disp(['Mean Absolute Value (MAV) for the entire signal (Myopathy Signal): ', num2str(mavValueFullWindowMyopathy)]);
...
zcrFullWindowMyopathy = sum(filteredEMG_myopathy_offset(start_indexM:end_indexM-1) < 0 & filteredEMG_myopathy_offset(start_indexM+1:end_indexM) >= 0);
disp(['Zero Crossing Rate (ZCR) for the entire signal (Myopathy Signal): ', num2str(zcrFullWindowMyopathy)]);

%Compute the FFT of each unfiltered signal
fft_emg_healthy = fft(amplitudeHealthy);
fft_emg_myopathy = fft(amplitudeMyopathy);

%Compute the FFT of each filtered signal
fft_filteredEMG_healthy = fft(filteredEMG_healthy);
fft_filteredEMG_myopathy = fft(filteredEMG_myopathy);

%Compute frequency spectrum of each signal
[frequencySpectrum_healthy, psd_healthy] = compute_spectrum(filteredEMG_healthy, SAMPLING_RATE);
[frequencySpectrum_myopathy, psd_myopathy] = compute_spectrum(filteredEMG_myopathy, SAMPLING_RATE);
...
[mavValuesMyopathy, zcrValuesMyopathy, wlValuesMyopathy, windowTimesMyopathy, envelopeMyopathy, rmsValuesMyopathy] = calculate_window_metrics(rectified_myopathy, filteredEMG_myopathy_offset, timeMyopathy, SAMPLING_RATE);
...
smoothedEnvelopeMyopahty = movmean(envelopeMyopathy, 0.5 * SAMPLING_RATE); % Use a 0.5 second moving average window
...
subplot(2,1,2);
plot(timeMyopathy, amplitudeMyopathy, 'r', 'LineWidth', 1.5); 
xlabel('Time (s)');
ylabel('Amplitude (mV)');
title('Raw EMG Signal (Myopathy)');
xlim([0,12.715]);
ylim([-100,200]);
grid on;
...
subplot(2,1,2);
plot(timeMyopathy, rectified_myopathy, 'r', 'LineWidth', 1.5);  
xlabel('Time (s)');
ylabel('Amplitude (mV)');
title('Rectified Signal (Myopathy)');
xlim([0,12.715]);
grid on;
...
subplot(2,1,2);
plot(timeMyopathy, smoothedEnvelopeMyopahty, 'r', 'LineWidth', 1.5); 
xlabel('Time (s)');
ylabel('Amplitude (mV)');
title('RMS Envelope (Myopathy)');
xlim([0,12.715]);
grid on;
...
subplot(2,1,2);
plot(windowTimesMyopathy, rmsValuesMyopathy, 'r', 'LineWidth', 1.5);
xlabel('Time (s)');
ylabel('RMS Amplitude (mV)');
title('RMS Amplitude over Time (Sliding Window - Myopathy)');
xlim([0,12.715]);
grid on;
...
subplot(2,1,2);
plot(windowTimesMyopathy, mavValuesMyopathy, 'r', 'LineWidth', 1.5);
xlabel('Time (s)');
ylabel('MAV (mV)');
title('Mean Absolute Value (MAV) over Time - Myopathy');
xlim([0,12.715]);
grid on;
...
subplot(2,1,2);
plot(windowTimesMyopathy, zcrValuesMyopathy, 'r', 'LineWidth', 1.5);
xlabel('Time (s)');
ylabel('Zero Crossing Rate');
title('Zero Crossing Rate (ZCR) over Time - Myopathy');
xlim([0,12.715]);
grid on;
...
subplot(2,1,2);
plot(windowTimesMyopathy, wlValuesMyopathy, 'r', 'LineWidth', 1.5);
xlabel('Time (s)');
ylabel('Waveform Length');
title('Waveform Length (WL) over Time - Myopathy');
xlim([0,12.715]);
grid on;
...
subplot(2,1,2);
plot(frequencySpectrum_myopathy, psd_myopathy, 'r', 'LineWidth', 1.5);
xlabel('Frequency (Hz)');
ylabel('Power/Frequency (dB/Hz)');
title('Frequency Spectrum and Power Spectral Density (Myopathy)');
xlim([0,500]);
grid on;
